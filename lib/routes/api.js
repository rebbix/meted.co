...
var calculateRiskScores = require('../riskScores');
...
module.exports = (app) => {
  ...
  app.get('/api/me/stats', (req, res, next) => {
    ...
    // Add paging later
    db.table("photo").orderBy({index: db.desc("viewCount")}).filter(db.row("viewCount").gt(1)).limit(50).default([])
      .then(photos => {
        return _.map(photos, p => {
          p.license = licenses(p.license);
          p.riskLevel = calculateRiskScores(p.risks);
          return p;
        })
      })
